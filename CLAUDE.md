...
**Kelly distribution rule (hard): Kelly sizes ONLY against (a) the distribution the PM explicitly stated for THIS trade, or (b) the market distribution — never a synthesised edge.** There is no conviction/target-implied curve anywhere (the old `view_implied_distribution` + `kelly_conviction` were removed). `market_distribution(spot, fwd, vol, T)` (lognormal at the forward, ATM width — no edge ⇒ Kelly ≈ 0) is the elicitation's seed and the fallback. A stated curve is tagged `kelly_curve_key = (pair, horizon_days)` and consumers use it only for that trade (`curve_for_trade`; `AgentSession.stated_curve_for`); an **unedited** elicitation counts as *not stated* (clears the curve). `SizingSpec.distribution_source` / `StandardPack.kelly_distribution_source` = `"explicit" | "market"`; market is surfaced to the PM (Trade View warning; agent pack `DISTRIBUTION: MARKET` line — the model must say sizes reflect no stated view). The elicitation also renders on the Trade View **results** screen so switching to Kelly post-run has somewhere to state the curve. The agent's `run_standard_pack` now passes the sizing regime to `build_pack` (it previously dropped it → the agent was always fixed-loss). Known gap: Tier-2 `price_structure` still sizes fixed-loss under Kelly. The legacy `compute_sizing` conviction "Kelly fraction" is not user-facing (only its stop level reaches the agent). Guards: `tests/test_kelly_ui_helpers.py`, `tests/test_agent_sizing_regime.py`, `tests/test_kelly_ranking_stability.py`.
