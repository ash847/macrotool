...
- **Agent config parity with Trade View — PARTIALLY FIXED (2026-09-22).** Diagnosed from a real PM complaint (a tester on a personal scenario-weights profile reported the Agent giving different recommendations than Trade View for the same view).
  - ✅ **Fixed:** `_render_agent()` (`interface/app.py`) now refreshes `structure_constraint` / `primary_objective` / `trade_management` on the `AgentSession` every rerun, alongside `target_rr` / `sizing_method` / Kelly params (which were already live). Previously these three were captured **once**, at Agent-tab session creation, and never refreshed — so changing a sidebar preference dropdown mid-conversation left the Agent silently scoring on stale preferences while Trade View re-scored live. `AgentSession.cache_key()` already includes all three, so the fix correctly forces a fresh `build_pack()` on change rather than serving a stale cached pack.
  - ❌ **Still open — `user_email` is not threaded to the Agent at all.** `AgentSession` has no `user_email` field, and `agentic/tools.py: _run_standard_pack`'s `build_pack(...)` call never passes one — so the Agent **always** scores under the **global** scenario-weights profile, regardless of who's logged in. Trade View / Batch pass `user_email` (`flow.user_email = USER_EMAIL` in `interface/app.py`) and resolve the PM's **personal** profile when they're on the `personal_weights_emails` allowlist. **Why this needs fixing:** for any allowlisted PM (confirmed: 2 users currently), the Agent's ranking will *permanently and deterministically* diverge from Trade View's for every view they run — not an edge case, a guaranteed mismatch for exactly the PMs most invested in tuning their own weights. Fix: add `user_email` to `AgentSession`, populate from `current_user_email()` in `_render_agent()` / `_render_trade_chat()`, thread into the `build_pack()` call in `agentic/tools.py`.
...
- **Agent can't persist a spoken preference into the actual scoring gate.** A PM can type "no tail risk" or "avoid complex structures" straight into the Agent chat, and the LLM will narrate around the existing `CONTEXT GUIDANCE` commentary as if it's honouring that — but nothing in `agentic/` updates `structure_constraint` (or the other PM-preference fields) from conversation; there's no `[PREF_CHANGE]`-style mechanism like the legacy INTAKE flow has. Only the Trade View sidebar widgets actually change the scoring gate. **Why this needs fixing:** it's a believability gap, not just a parity one — the Agent can sound like it applied a hard constraint (excluding seagulls, say) when it only ever saw the *default* regime's natural aversion to that structure, and a genuinely gated-out structure could resurface next turn once the regime commentary changes. Decide the product answer first (should the Agent be able to set preferences at all, and if so where do they live — session-only or synced back to the sidebar?) before wiring a mechanism.
