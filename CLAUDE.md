...
workspace/      Saved conversations + trade ideas — models, identity rule, store seam, resume/refresh
db/             SQL to run once in Supabase (workspace_schema.sql)
...
- `agent_llm.py` — the `ToolLLM` seam (`AnthropicToolLLM` + `FakeToolLLM` for no-API tests). Default model **`claude-sonnet-4-6`**, `MAX_TOKENS=9000` (raised from 2048 alongside `_TOP_N=5` below — 5 structures at this PM base's usual table+prose+advantages/drawbacks depth was pushing observed replies close to the old cap; input side is a non-issue, a rendered pack with 5 structures is only ~2K tokens against a 200K window).
...
- `standard_pack.py: build_pack()` → `StandardPack`; `render.py: render_pack()` projects it to **labelled text** (VIEW, MARKET CONTEXT, CONTEXT GUIDANCE, RECOMMENDED STRUCTURES with per-leg breakdown + driver split, SIZING stop). The LLM narrates over this text and never sees raw objects. `render.py: _TOP_N = 5` — the pack computes every eligible structure (no fixed count, whatever clears the affinity gates for that market state) but only the top-N by PnL score are rendered in full; the rest are collapsed into a bare count ("+N more considered") with identities withheld unless the PM names one and it's priced via `price_structure`.
...
## Saved conversations (`workspace/`)

PMs can reopen past Agent conversations and carry on. **Phase 1 = option A: one conversation, one *active* idea** — the last view the PM ran a pack on. It labels the chat (auto-title, e.g. `USDJPY ↑ 162.35 · 18-Dec-26`, unless the PM renamed it). The active idea can change mid-chat (BRL ↓ → JPY ↑ → the chat is now "JPY ↑"); there is no memory across ideas and the agent never context-switches on its own. New chats are explicit (sidebar **＋ New conversation**).

- **`ViewSpec` stores an absolute target level + expiry date**, never `horizon_days`/`magnitude_pct`. Re-evaluation rebuilds the view with `horizon = expiry − snapshot_date` and passes `target_level` to `run_standard_pack`, so magnitude is re-derived off *that day's* forward. **The clock is the snapshot date, not wall-clock** — a static snapshot reproduces the same trade exactly.
- **Identity rule** (`identity.classify_view_change`): an idea = **pair + direction**. Target / expiry / mode / PM-pref changes (and new data or engine output for the same view) → new `IdeaVersion` of the same idea. Returning to a pair+direction in the same chat reuses its idea.
- **Tables** (`db/workspace_schema.sql`, run once in Supabase): `trade_ideas`, `idea_versions`, `conversations` (`active_idea_id`), `conversation_ideas` (every idea a chat touched — unused by the UI now; it is the join option C needs, so C is a UI change, not a migration), `conversation_turns` (per exchange: display text + the **exact provider messages** as JSON — the replay source). Every table carries `user_email`; **`workspace/store.py` filters every read on it** (the app writes with the service key, which bypasses RLS — that filter is the privacy guard). RLS is on with no policies (anon key sees nothing).
- **Store seam:** `SupabaseStore` / `InMemoryStore`. The UI falls back to in-memory (chats last the browser session, with a caption) if Supabase or the tables are missing — never crashes the chat. `InMemoryStore` mirrors the SQL foreign keys so tests catch write-ordering bugs; writes go idea → version → conversation → link → turn (`ConversationService._persist`).
- **Resume** (`ConversationService.resume`): replay stored messages verbatim, then rebuild the active idea's pack on the **current** snapshot via the real `run_standard_pack` path (so `price_structure` works immediately). If the rebuilt render ≠ the last saved one → *stale*: banner, and the next question first runs `refresh`, which injects a synthetic tool turn whose result starts **`REFRESHED — active trade: …`** (system prompt: it supersedes every earlier pack, any pair). Unchanged → nothing is added ("No new market data since last evaluation"). Expired ideas and targets now through the forward surface a note instead. Failed exchanges persist display text but no provider messages, so replayed history is never half a tool loop.
- **Per-chat settings** (`workspace/settings.py: ChatSettings`, `interface/agent_settings_ui.py`): each chat has its own sizing method, λ, R:R and PM preferences (the merged Trade View menu) + its own stated distributions (`{curve_key: {probs, bins}}`) — stored on `conversations.settings` / `.distributions` (`db/workspace_schema_002_chat_settings.sql`; if those columns are missing the store saves without them and `settings_persist=False`). **Only capital W is global.** New chats inherit the last-used chat's settings; Trade View keeps its own (the Agent no longer follows Trade View). A Kelly chat sizes the active trade under Kelly only if a distribution exists for its pair + expiry — otherwise fixed-loss + flag; switching trades mid-chat picks the matching distribution (or falls back), switching back restores it. The strip = one-line summary ("Kelly selected — no distribution for USDJPY 24-May-26 → fixed-loss" etc.) + a "✎ Sizing & preferences" editor (distribution inputs for the active trade reuse `render_kelly_elicitation(key_prefix=…, write_session=False, seed_curve=…)`); **Apply** → `ConversationService.apply_settings` re-runs the active trade and injects a `SETTINGS UPDATED — …` pack turn (system prompt: supersedes earlier figures) + a `settings` turn. `resume(conv_id, make_session(conv))` builds the session from the chat's settings.
- **Background turns** (`interface/agent_job.py`): each Agent turn (`flow.advance` + `record_exchange` + telemetry) runs in a **daemon thread**, never inline. Streamlit stops a script run at its next `st.*` call when the user clicks elsewhere, so an inline turn lost its reply (shown nowhere, saved nowhere, yet already in the model's history). The page only *waits* (a placeholder loop — each update is an interruptible `st` call) and `_finalize_agent_jobs` applies the result on the next Agent render. Jobs are keyed by conversation id; seq numbers (turn + telemetry) are reserved at submit. The thread must not call `st` (no script context). Refresh/⋯ are disabled while a turn runs. Guard: `tests/test_agent_job.py`.
- **UI:** sidebar **Previous conversations** list on **every page** (click → opens on Agent; highlighted only while on Agent; list cached per open-chat state + 60s TTL since Trade View reruns constantly). The nav button reads **"New Chat"** and always starts a fresh chat (page id stays `"Agent"`). Header (title, as-of date, ↻ Refresh, ⋯ rename/archive) + `?chat=<id>` deep link. Archive is a soft delete. Existing `chat_turns` telemetry is unchanged (separate purpose); the Trade View "Ask about this trade" chat is **not** saved yet (would be `surface="trade_view"`).
- Guard: `tests/test_workspace.py` (spec round-trip, identity rule, serialisation of SDK blocks, store privacy, active-idea tracking, resume/refresh/stale/expired, SupabaseStore via a fake PostgREST client).
...
**Kelly distribution rule (hard): Kelly sizes a trade ONLY against a distribution the PM explicitly stated for THAT trade (pair + expiry date). No stated distribution → the trade is sized FIXED-LOSS and flagged — never a synthesised edge.** There is no conviction/target-implied curve anywhere (`view_implied_distribution` / `kelly_conviction` removed). `market_distribution(spot, fwd, vol, T)` (lognormal at the forward) is only the elicitation's starting point; an **unedited** elicitation counts as *not stated*. Distributions are keyed `curve_key(pair, expiry) = "PAIR|YYYY-MM-DD"` — the **expiry date, not horizon days**, so a saved view stays attached as time passes; `curve_for_trade(curves, pair, expiry)` is the only lookup. `SizingSpec.kelly_fallback` / `StandardPack.kelly_fallback` mark "Kelly selected, no distribution → fixed-loss": Trade View shows `KELLY_FALLBACK_MSG`; the agent pack's `SIZING REGIME: FIXED-LOSS — the PM selected KELLY, but has NOT set up a distribution for PAIR DD-Mon-YY` line (system-prompt exception to the regime lock) makes the model lead with it and ask the PM to set one up. The Trade View elicitation also renders on the results screen. The agent's `run_standard_pack` passes the regime (+ `user_email` for personal weights) to `build_pack`; Tier-2 `price_structure` sizes with the pack's `sizing_spec`. The legacy `compute_sizing` conviction "Kelly fraction" is not user-facing. Guards: `tests/test_kelly_ui_helpers.py`, `tests/test_agent_sizing_regime.py`, `tests/test_kelly_ranking_stability.py`.
