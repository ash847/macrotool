...
- **UI:** sidebar list (Agent page) + header (title, as-of date, ↻ Refresh, ⋯ rename/archive) + `?chat=<id>` deep link. Archive is a soft delete. Existing `chat_turns` telemetry is unchanged (separate purpose); the Trade View "Ask about this trade" chat is **not** saved yet (would be `surface="trade_view"`).
