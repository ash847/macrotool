...
- **UI:** sidebar **Conversations** list on **every page** (click → opens on Agent; highlighted only while on Agent; list cached per open-chat state + 60s TTL since Trade View reruns constantly). The nav button reads **"Agent (new chat)"** and always starts a fresh chat (page id stays `"Agent"`). Header (title, as-of date, ↻ Refresh, ⋯ rename/archive) + `?chat=<id>` deep link. Archive is a soft delete. Existing `chat_turns` telemetry is unchanged (separate purpose); the Trade View "Ask about this trade" chat is **not** saved yet (would be `surface="trade_view"`).
